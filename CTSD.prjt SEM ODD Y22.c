...
void insert_details()
{
	...
	{
		fprintf(fp,"%s %s %s %d \n",b.code,b.name,b.date,b.cost);//fprintf(fp," ",);fscanf(fp," ",&)
...
void viewAll()
{
	...
	{	
		...
		while( ( ch = fgetc(fp) ) != EOF )//End of file(EOF)
      		printf("%c",ch);
